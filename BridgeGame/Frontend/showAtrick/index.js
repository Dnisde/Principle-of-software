...
function displayCard() {
    ...
    var divActivate = ["trick-input-w", "trick-btn-w", "trick-input-e", "trick-btn-e", "trick-input-s", "trick-btn-s"]
    for (var i = 0; i < divActivate.length; i++) {
        document.getElementById(divActivate[i]).disabled = false;
    }
}
...
function createCardDiv(num, cardInfo) {
    ...
    cardClickArea.onclick = function () {
        alert("Select this card");
        cardTemp.style.filter = "brightness(0.8)";
        cardTemp.style.pointerEvents = 'none';
        let cardNorth = document.getElementById("card-north");
        cardNorth.setAttribute("src", url_card);
        for (var i = 0; i < cardDock.length; i++) {
            document.getElementById("p1card" + (i + 1)).style.pointerEvents = 'none';
        }
    }
...
function parseTrickwest() {
    let cardString = document.getElementById("trick-input-w").value;
    let cardInfo = [];
    cardInfo = cardString.split(' ');
    // Check repeated cards
    if (cardTotal.includes(cardString)) {
        alert("This card is owned by other players! ")
    }
    else if (cardInfo.length != 2) {
        alert('Invalid input length!');
    }
        ...
        const suit = cardInfo[1].toLowerCase();
        let url_card = "../all_cards/" + cardInfo[0] + "_" + cardInfo[1] + ".png";
        let cardBack = document.getElementById("card-west");
        if (!allRanks.includes(rank) || !allSuits.includes(suit)) {
            alert('This card input is not valid!');
        }
        else if (cardBack) {
            ...
            if (cardBack.width == 0) {
                ...
                cardTotal.push(cardString);
                document.getElementById("trick-input-w").disabled = true;
                document.getElementById("trick-btn-w").disabled = true;
            }
        }
    }
}

function parseTrickeast() {
    let cardString = document.getElementById("trick-input-e").value;
    let cardInfo = [];
    cardInfo = cardString.split(' ');
    // Check repeated cards
    if (cardTotal.includes(cardString)) {
        ...
        const suit = cardInfo[1].toLowerCase();
        let url_card = "../all_cards/" + cardInfo[0] + "_" + cardInfo[1] + ".png";
        let cardBack = document.getElementById("card-east");
        if (!allRanks.includes(rank) || !allSuits.includes(suit)) {
            alert('This card input is not valid!');
        }
        else if (cardBack) {
            ...
            if (cardBack.width == 0) {
                ...
                cardTotal.push(cardString);
                document.getElementById("trick-input-e").disabled = true;
                document.getElementById("trick-btn-e").disabled = true;
            }
        }
    }
}

function parseTricksouth() {
    let cardString = document.getElementById("trick-input-s").value;
    let cardInfo = [];
    cardInfo = cardString.split(' ');
    // Check repeated cards
    if (cardTotal.includes(cardString)) {
        ...
        const suit = cardInfo[1].toLowerCase();
        let url_card = "../all_cards/" + cardInfo[0] + "_" + cardInfo[1] + ".png";
        let cardBack = document.getElementById("card-south");
        if (!allRanks.includes(rank) || !allSuits.includes(suit)) {
            alert('This card input is not valid!');
        }
        else if (cardBack) {
            ...
            if (cardBack.width == 0) {
                ...
                cardTotal.push(cardString);
                document.getElementById("trick-input-s").disabled = true;
                document.getElementById("trick-btn-s").disabled = true;
            }
        }
    }
}